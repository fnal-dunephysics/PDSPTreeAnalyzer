...
void PionStudy::executeEvent(){
  ...
  */
  //cout << "[PionStudy::executeEvent] Passed all beam selections" << endl;

  Study_with_daughters(P_reweight);
}

void PionStudy::True_KI_study(TString prefix){
...
void PionStudy::Study_with_daughters(double weight){
...
//  }
  ...
  for (auto const & pion : pions){
    double KE_hypfit_gaus = KE_Hypfit_Gaussian(pion, 211);
    double KE_hypfit_like = KE_Hypfit_Likelihood(pion, 211);
    if (KE_hypfit_like > maxKE){
      maxKE = KE_hypfit_like;
    ...
    if(!IsData){
      double true_E = pion.PFP_true_byHits_startE() * 1000.;
      double true_KE = true_E - M_pion;
      if (KE_hypfit_gaus>0){
        JSFillHist("pion", "hypfit_gaus", true_KE, KE_hypfit_gaus, 1.,
                   100,0,1000,100,0,1000);
      }
      if (KE_hypfit_like>0){
        JSFillHist("pion", "hypfit_like", true_KE, KE_hypfit_like, 1.,
                   100,0,1000,100,0,1000);
      }
    }
  }
  ...
  maxKE = -1;
  for (auto const & proton : stopping_protons){
    double proton_KE = map_BB[2212] -> KEFromRangeSpline(proton.allTrack_resRange_SCE().back());
    double proton_KE_fit = KE_Hypfit_Gaussian(proton, 2212);
    if (proton_KE > maxKE){
      maxKE = proton_KE;
      double E = proton_KE+M_proton;
      double p = sqrt(pow(E,2)-pow(M_proton,2));
      ...
      double px = p*p_vec_proton.X();
      double py = p*p_vec_proton.Y();
      double pz = p*p_vec_proton.Z();
      Proton.SetPxPyPzE(px, py, pz, E);
    }
    if(!IsData){
      double true_E = proton.PFP_true_byHits_startE() * 1000.;
      double true_KE = true_E - M_proton;
      if (proton_KE>0){
        JSFillHist("proton", "range", true_KE, proton_KE, 1.,
                   100,0,500,100,0,500);
      }
      if (proton_KE_fit > 0){
        JSFillHist("proton", "hypfit", true_KE, proton_KE_fit, 1.,
                   100,0,500,100,0,500);
      }
    }
  }
  TLorentzVector Ar(0,0,0,37.2247242e3);
  ...
  double mn = 939.565;
  double mpi = 139.57;
  double mar = 37.2247242e3;
  double Eb = 4;

  if (Pion.M()){
    double Pz = (Pion.Px()*beam_px+Pion.Py()*beam_py+Pion.Pz()*beam_pz)/beam_p;
    double Eqe = (pow(mp,2)-pow(mn-Eb,2)-pow(mpi,2)+2*(mn-Eb)*Pion.E())/2/(mn-Eb-Pion.E()+Pz);
    JSFillHist("KI", "Eqe", Beam.E(), Eqe, 1.,
               100,0,1000,100,0,1000);
  }
  if (Pion.M() && Proton.M()){
    double Mx = (Beam + Ar - Pion - Proton).M()/1000;
//    Beam.Print();
//    Ar.Print();
//    Pion.Print();
//    Proton.Print();
//    cout<<Mx<<endl;
    JSFillHist("KI","Mx", Mx, 1, 100,35,40);
    double pf = (Pion+Proton-Beam).P();
    JSFillHist("KI","pf", pf, 1, 100,0,1000);
    
  }
}

void PionStudy::Beam_true_Eloss(){
...
void PionStudy::FillBeamPlots(TString beam_selec_str, double weight){
...
void PionStudy::drawKIVar(Daughter pion, Daughter proton, double weight){
...
double PionStudy::KE_to_P(double KE, int PID){
...
bool PionStudy::Pass_beam_start_Z_cut(double N_sigma){
...
bool PionStudy::Pass_beam_delta_X_cut(double N_sigma){
...
bool PionStudy::Pass_beam_delta_Y_cut(double N_sigma){
...
bool PionStudy::Pass_beam_TPC_theta_cut(double N_sigma){
  bool out = false;
  double Beam_TPC_theta_over_sigma = (beam_TPC_theta - Beam_TPC_theta_mu_data) / Beam_TPC_theta_sigma_data;
...
bool PionStudy::Pass_beam_TPC_phi_cut(double N_sigma){
  bool out = false;
  double Beam_TPC_phi_over_sigma = (beam_TPC_phi - Beam_TPC_phi_mu_data) / Beam_TPC_phi_sigma_data;
...
void PionStudy::FillTKIVar(TString dir, TVector3 p_vec_beam, TVector3 p_vec_f1, TVector3 p_vec_f2, double weight){
