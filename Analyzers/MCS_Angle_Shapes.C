#include "MCS_Angle_Shapes.h"

void MCS_Angle_Shapes::initializeAnalyzer(){
...
void MCS_Angle_Shapes::executeEvent(){
...
void MCS_Angle_Shapes::Run_beam_MCS(){
  ...
  for(unsigned int i_true_hit = 0; i_true_hit < (*evt.true_beam_traj_Z).size(); i_true_hit++){
    ...
    //cout << Form("[MCS_Angle_Shapes::Run_beam_MCS] %d this_P : %.2f", i_true_hit, this_P) << endl;
  }
  ...
  for(unsigned int i_reco_hit = 0; i_reco_hit < reco_Z.size(); i_reco_hit++){
    ...
    //cout << Form("[MCS_Angle_Shapes::Run_beam_MCS] reco %d : (%.2f, %.2f, %.2f)", i_reco_hit, reco_X.at(i_reco_hit), reco_Y.at(i_reco_hit), reco_Z.at(i_reco_hit)) << endl;
  }
  ...
  //cout << "[MCS_Angle_Shapes::Run_beam_MCS] " << evt_run_str << ", test_fit.Mag() : " << test_fit.Mag() << endl;
...
void MCS_Angle_Shapes::Run_beam_MCS_for_Segments(const vector<TVector3> & reco_position_vec, const vector<TVector3> & true_position_vec, const vector<double> & true_P_vec, double segment_size, TString name){
...
void MCS_Angle_Shapes::Run_Daughter_MCS(const vector<Daughter>& pions){
...
void MCS_Angle_Shapes::Run_Daughter_MCS_for_Segments(const vector<TVector3> & reco_position_vec, double true_P, int this_PdgID, double segment_size, TString name){
  ...
  */
  ...
  //cout << "[MCS_Angle_Shapes::Run_Daughter_MCS_for_Segments] " << name << ", theta_yz : " << theta_yz << ", theta_xz : " << theta_xz << ", theta_3D : " << theta_3D << endl;
...
vector<double> MCS_Angle_Shapes::GetSegmentTrueP(const vector<MCSSegment> & segments, const vector<TVector3> & true_position_vec, const vector<double> true_P_vec, int PDG){
  vector<double> out;
...
MCS_Angle_Shapes::MCS_Angle_Shapes(){
